...
function PortFolio2() {
  ...
  if (timeseries) {
    const secondArray = timeseries[1];
    const data =
      secondArray?.map(({ datetime, close }) => ({
        index: datetime,
        pv: close,
      })) || [];
    ...
    <div>
      <div className="group flex flex-col justify-center hover:bg-slate-200 rounded-xl p-3 md:p-5">
        <div className="flex justify-between items-center">
          <button className="py-3 flex justify-center items-center rounded-md border border-transparent font-semibold  hover:bg-amber-400 focus:outline-none text-sm bg-sky-400">
            <img src="google.png" alt="logo" className="w-6 h-6" />
            <span className="px-2 text-white">Alphabet Inc</span>
          </button>

          <div>
            <span className="font-bold text-sm text-gray-500">
              {livepricesarray.symbols[1]}
            </span>
          </div>
        </div>
        ...
        <div className="mt-3 flex">
          <div className="flex flex-col flex-nowrap">
            ...
            {livepricesarray.price[1] ? (
              <p className="font-bold text-md">
                ${Number(livepricesarray?.price[1]).toFixed(2)}
              </p>
            ) : (
            ...
            <span className="mt-5 relative text-md text-blue-600 font-medium cursor-pointer">
              Overview
              <span className="absolute left-0 bottom-0 w-full h-0.5 bg-blue-600 transform origin-bottom scale-x-0 transition-transform group-hover:scale-x-100"></span>
            </span>
          </div>
